...
class UserConfig {
   ...
   </ label="Select listbox, vert_wheel", help="Select wheel type or listbox", options="vert_wheel", order=2 /> enable_list_type="vert_wheel";
   </ label="Select spinwheel art", help="The artwork to spin", options="wheel", order=3 /> orbit_art="wheel";
   ...
   </ label="Select pointer", help="Select animated pointer", options="none", order=6 /> enable_pointer="none";  
</ label="--------    Miscellaneous    --------", help="Miscellaneous options", order=7 /> uct6="select below";
   </ label="Background", help="Background", options="Backdrop", order=8 /> enable_cab="Backdrop";
   </ label="Enable monitor static effect", help="Show static effect when snap is null", options="No", order=9 /> enable_static="No";
}  
...
local surface_snap = fe.add_surface( 1280, 720 );
local snap = FadeArt("snap", 0, 0, 1280, 720, surface_snap);
snap.trigger = Transition.EndNavigation;
snap.preserve_aspect_ratio = false;

//now position and pinch surface of snap
//adjust the below values for the game video preview snap
surface_snap.set_pos(flx*0.2875, fly*0.3194, flw*0.59375, flh*0.5972 );
surface_snap.skew_y = 0;
surface_snap.skew_x = 2.6;
...
//adjust the values below for the static preview video snap
   ...
   local snapbg=null;
   snapbg = fe.add_image( "static.mp4", flx*0.2875, fly*0.3194, flw*0.59375, flh*0.5972 );
...
//cabinet image
if ( my_config["enable_cab"] == "Backdrop" )
{
 local cab = fe.add_image( "Backdrop.png", 0, 0, flw, flh );
}
